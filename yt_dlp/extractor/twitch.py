...
class TwitchBaseIE(InfoExtractor):
    ...
    _OPERATION_HASHES = {
        ...
        'VideoComments': 'be06407e8d7cda72f2ee086ebb11abb6b062a7deb8985738e648090904d2f0eb',
...
class TwitchVodIE(TwitchBaseIE):
    ...
    def _get_subtitles(self, vod_id):
        gql_ops = [{
            ...
            'variables': {'hasVideoID': True, 'videoID': vod_id},
        }]
        for retry in self.RetryManager():
            response = self._download_gql(vod_id, gql_ops, 'Downloading chat', fatal=False)

            if response is False:
                retry.error = ExtractorError("f'Unable to fetch next chat.'", video_id=vod_id, ie=self)

        response_errors = traverse_obj(response, (..., 'errors'))
        if response_errors:
            self.report_warning(f'Error response received for fetching chat: {response_errors}')

        data_obj = traverse_obj(response, (0, 'data'))
        ...
        while has_more_pages:
            ...
            response_errors = traverse_obj(response, (..., 'errors'))
            if response_errors:
                self.report_warning(f'Error response received for fetching next chat history fragment: {response_errors}')
        ...
        data_obj['comments'] = chat_history
    ...
    def _real_extract(self, url):
        ...
        if info.get('timestamp'):
            info['__post_extractor'] = lambda: {'requested_subtitles': {'rechat': rechat}} if (rechat := traverse_obj(self.extract_subtitles(vod_id), ['rechat', 0])) else {}
